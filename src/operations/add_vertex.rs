...
use crate::graph::graph::graph::{Graph, GraphTrait, VertexIndex};
use crate::graph::graph::vertex_store::SetVertexData;
use crate::graph::value_type::{NativeDataType, implement_macro_for_all_native_value_types};
...
macro_rules! implement_add_vertex {
    ($value_type:ty) => {
        impl AddVertex<$value_type> for Graph<$value_type> {
            fn add_new_vertex(&mut self, vertex: Vertex<$value_type>) -> Result<VertexIndex, GraphComputingError> {
                self.vertex_store_mut_ref().add_new_vertex(vertex)
            }

            fn add_or_replace_vertex(&mut self, vertex: Vertex<$value_type>) -> Result<VertexIndex, GraphComputingError> {
