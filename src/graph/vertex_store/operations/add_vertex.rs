...
pub(crate) trait AddVertex<T: ValueType> {
    ...
    fn add_or_replace_vertex(&mut self, vertex: Vertex<T>) -> Result<Index, GraphComputingError>;
    ...
    ($value_type:ty) => {
        impl AddVertex<$value_type> for VertexStore<$value_type> {
            fn add_new_vertex(
                ...
                let index = self.indexer_mut_ref().add_new_key(vertex.key_ref())?;
            ...
            fn add_or_replace_vertex(
                ...
                let index = self
                    .indexer_mut_ref()
                    .add_or_replace_key(vertex.key_ref())?;
                self.vertex_matrix_mut_ref()
                    .set_element((index, index, vertex.value_ref().clone()).into())?;
                Ok(index)
            }
