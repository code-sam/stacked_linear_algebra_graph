...
use super::{AtomicInMemoryIndexerTransaction, GetIndexerStateRestorer, GetIndexerUnderTransaction, RegisterNewIndexToRevert};
...
impl<'a> GeneratePublicIndex for AtomicInMemoryIndexerTransaction<'a> {
    fn new_public_index(&mut self) -> Result<AssignedIndex, GraphComputingError> {
        ...
        self.indexer_state_restorer_mut_ref().register_new_public_index_to_revert(&index)?;
        Ok(index)
    }
}
...
impl<'a> GeneratePrivateIndex for AtomicInMemoryIndexerTransaction<'a> {
    fn new_private_index(&mut self) -> Result<AssignedIndex, GraphComputingError> {
        ...
        self.indexer_state_restorer_mut_ref().register_new_private_index_to_revert(&index)?;
        Ok(index)
    }
}
