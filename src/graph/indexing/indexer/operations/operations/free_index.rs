...
pub(crate) fn free_public_index(
    ...
    if indexer.is_valid_public_index(index)? {
        indexer.free_public_index_unchecked(index)
    } else {
        Ok(())
    }
}
...
pub(crate) fn free_private_index(
    ...
    if indexer.is_valid_private_index(index)? {
        indexer.free_private_index_unchecked(index)
    } else {
        Ok(())
    }
}
