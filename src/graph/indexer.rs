...
pub(crate) trait IndexerTrait {
    fn add_new_key(&mut self, key: &KeyRef) -> Result<Index, GraphComputingError>;
    fn add_or_replace_key(&mut self, key: &KeyRef) -> Result<Index, GraphComputingError>;
...
impl IndexerTrait for Indexer {
    ...
    fn key_for_index(&self, index: &Index) -> Result<Key, GraphComputingError> {
        match self.is_valid_index(index)? {
            true => Ok(self.index_to_key_map[index.clone()].clone()),
            false => {
                return Err(LogicError::new(
...
impl Indexer {
    ...
    fn claim_available_index(&mut self) -> Result<Index, GraphComputingError> {
